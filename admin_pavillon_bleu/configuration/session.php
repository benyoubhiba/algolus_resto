...
$emailuser=$requset['Mail'];
   ...
   if($login=='' || $password==''){
       header('location: ../app/page-authentification.php?error=1');
   }
   elseif ($password==''){
       header('location: ../app/page-authentification.php?error=2');

   }
   elseif ($password!==$passworduser|| $login!==$loginuser && $login!==$emailuser){
   header('location: ../app/page-authentification.php?error=3$password=' .$password);
